...
export class TransportRequestPersistComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private transportRequestService: TransportRequestService, private router: Router, private locationService: LocationService, private locationService: LocationService, private consignmentService: ConsignmentService, private customerService: CustomerService) {}
  ...
  ngOnInit() {
    ...
    this.consignmentService.list().subscribe((consignmentList: Consignment[]) => { this.consignmentList = consignmentList; });
    this.customerService.list().subscribe((customerList: Customer[]) => { this.customerList = customerList; });
